...
class UserService
{
    ...
    public function getCustomFieldTemplateCustomer(): string
    {
        $request = $this->requestStack->getCurrentRequest();
        $ticket = $this->entityManager->getRepository(Ticket::class)->find($request->attributes->get('id'));

        if (true === $this->isfileExists('apps/uvdesk/custom-fields')) {
            ...
            if (true === $this->isfileExists('apps/uvdesk/form-component')) {
    ...
    public function getSessionUser(): null|UserInterface|User
    {
    ...
    public function getCurrentUser(): false|UserInterface|User
    {
    ...
    public function isAccessAuthorized($scope, User $user = null): bool
    {
        ...
        switch ($userRole) {
            ...
            case 'ROLE_AGENT':
                ...
                return in_array($scope, $agentPrivileges);
            case 'ROLE_CUSTOMER':
            default:
                break;
        }
    ...
    {
        if (true === isset($this->agentPrivilege[$userId])) {
            return $this->agentPrivilege[$userId];
        }
        ...
        $this->agentPrivilege[$userId] = $userPrivileges;

        return $userPrivileges;
    }

    public function getSupportPrivileges(): array
    {
    ...
    public function createUserInstance($email, $name, SupportRole $role, array $extras = [])
    {
        ...
        if (null == $user->getId()) {
            ...
            $user->setEmail($email);
            $user->setFirstName($extras['firstName'] ?? array_shift($name));
        ...
        if (empty($userInstance)) {
            ...
            $userInstance->setIsActive(empty($extras['active']) ? false : $extras['active']);
            $userInstance->setIsVerified(empty($extras['verified']) ? false : $extras['verified']);
            $userInstance->setIsStarred(empty($extras['starred']) ? false : $extras['starred']);
    ...
    public function getAgentPartialDataCollection(Request $request = null): array
    {
        $queryBuilder = $this->entityManager->createQueryBuilder()
    ...
    public function getAgentsPartialDetails(Request $request = null): array
    {
        static $agents;
        if (null !== $agents) {
            return $agents;
        ...
        return $agents = $qb->getQuery()->getArrayResult();
    }
    ...
    public function getUsersByGroupId($groupId): array
    {
        $qb = $this->entityManager->createQueryBuilder();
    ...
    public function getUsersBySubGroupId($subGroupId): array
    {
        $qb = $this->entityManager->createQueryBuilder();
    ...
    public function getCustomerDetailsById($customerId)
    {
        $qb = $this->entityManager->createQueryBuilder();
        $qb->select(
            "user.id,user.email,CONCAT(user.firstName,' ', COALESCE(user.lastName,'')) AS name,user.firstName,user.lastName,user.isEnabled,userInstance.contactNumber,userInstance.profileImagePath,userInstance.profileImagePath as smallThumbnail,userInstance.isActive, userInstance.isVerified"
        )->from(User::class, 'user')
            ->leftJoin('user.userInstance', 'userInstance')
            ->andwhere('userInstance.supportRole = :roles')
    ...
    public function getCustomerPartialDetailById($customerId)
    {
        $qb = $this->entityManager->createQueryBuilder();
        $qb->select(
            "u.id,u.email,CONCAT(u.firstName,' ', COALESCE(u.lastName,'')) AS name,u.firstName,u.lastName,userInstance.contactNumber,userInstance.profileImagePath,userInstance.profileImagePath as smallThumbnail"
        )->from(User::class, 'u')
            ->leftJoin('u.userInstance', 'userInstance')
            ->andwhere('userInstance.supportRole = :roles')
    ...
    public function getCustomersPartial(Request $request = null): string|array|int|float
    {
        $qb = $this->entityManager->createQueryBuilder();
    ...
    public function getUserSubGroupIds(int $userId): array
    {
        $qb = $this->entityManager->createQueryBuilder();
    ...
    public function getWebsiteConfiguration($code)
    {
        ...
        $website = $this->entityManager->getRepository(Website::class)->findOneBy(['code' => $code]);
    ...
    public function getWebsiteDetails($code): ?Website
    {
        return $this->entityManager->getRepository(Website::class)->findOneBy(['code' => $code]);
    }

    public function convertToTimezone(DateTimeInterface $date, $format = "d-m-Y H:ia"): string
    {
    ...
    public function convertToDatetimeTimezoneTimestamp(DateTimeInterface $date, $format = "d-m-Y h:ia"): int
    {
        $date = date_format($date, $format);
        $dateTime = date('Y-m-d H:i:s', strtotime($date));
        $scheduleDate = new DateTime($dateTime, new DateTimeZone(date_default_timezone_get()));
    ...
    public function removeCustomer(User $customer): void
    {
        $userData = $this->entityManager->getRepository(UserInstance::class)->findBy(['user' => $customer->getId()]);

        $count = count($userData);
        $ticketData = $this->entityManager->getRepository(Ticket::class)->findBy(['customer' => $customer->getId()]);
    ...
    public function removeAgent(User $user): void
    {
        $userData = $this->entityManager->getRepository(UserInstance::class)->findBy(['user' => $user->getId()]);
        $count = count($userData);
    ...
    public function getWebsiteView(): string
    {
        $website = $this->entityManager->getRepository(Website::class)->findOneBy(['code' => 'knowledgebase']);
    ...
    public function getUserPrivilegeIds(int $userId): array
    {
        $qb = $this->entityManager->createQueryBuilder();
    ...
    public function getWebsiteSpamDetails($websiteSpam): array
    {
    ...
    public function filterBlockSpam(string $str): array
    {
        $list = [];
    ...
    public function getTimezones(): array|false
    {
        return DateTimeZone::listIdentifiers();
    }

    public function getUserSavedReplyReferenceIds(): array
    {
        ...
        foreach ($result as $row) {
            if (in_array($row['group_id'], $groupIds)) {
                $savedReplyIds[] = (int)$row['savedReply_id'];
            }
        }
    ...
    public function getLocalizedFormattedTime(DateTime $timestamp, $user = null, $format = 'm-d-y h:i A'): string
    {
